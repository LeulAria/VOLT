...
const DETAILED_GROUPS = new Set(['pinned', 'today', 'results']);

/** Pinned and Today keep a title plus summary; older buckets are a single truncated line. */
export function isDetailedHistoryGroup(key: string): boolean {
	return DETAILED_GROUPS.has(key);
}
